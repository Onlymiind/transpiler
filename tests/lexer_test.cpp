...
TEST_CASE("lexer: booleans", "[lexer]") {
    ...
    for (auto c : cases) {
        ...
        REQUIRE(t.boolean == c.expected.boolean);
    }
}
...
TEST_CASE("lexer: punctuation", "[lexer]") {
    std::vector<TestCase> cases = {
        ...
        {",", common::Token{.type = common::TokenType::COMMA}},
    };
...
TEST_CASE("lexer: integers", "[lexer]") {
    std::vector<TestCase> cases = {
        {"1234", common::Token{.type = common::TokenType::INTEGER, .integer = 1234}},
        {"00012892743\t", common::Token{.type = common::TokenType::INTEGER, .integer = 12892743}},
    ...
    for (auto c : cases) {
        ...
        auto result = l.reset();
        REQUIRE(t.type == c.expected.type);
        REQUIRE(t.integer == c.expected.integer);
    }
}
...
TEST_CASE("lexer: floats", "[lexer]") {
    std::vector<TestCase> cases = {
        {"1234.1234",
         common::Token{.type = common::TokenType::FLOAT, .floating = 1234.1234}},
        {"00012892743.12345678\t", common::Token{.type = common::TokenType::FLOAT, .floating = 12892743.12345678}},
        {.str = "true", .should_fail = true},
        {.str = "", .should_fail = true},
        {.str = "+-", .should_fail = true},
    };
    ...
    for (auto c : cases) {
        ...
        auto result = l.reset();
        REQUIRE(t.type == c.expected.type);
        REQUIRE(std::abs(t.floating - c.expected.floating) <= epsilon);
    }
}
...
TEST_CASE("lexer: identifiers", "[lexer]") {
    ...
    std::vector<TestCase> cases = {
        {"trueuyt", common::Token{.type = common::TokenType::IDENTIFIER, .identifier = lit.add("trueuyt")}},
        {"false12456_", common::Token{.type = common::TokenType::IDENTIFIER, .identifier = lit.add("false12456_")}},
        {"_1234", common::Token{.type = common::TokenType::IDENTIFIER, .identifier = lit.add("_1234")}},
        {"id_id_id ", common::Token{.type = common::TokenType::IDENTIFIER, .identifier = lit.add("id_id_id")}},
        {"fun", common::Token{.type = common::TokenType::IDENTIFIER, .identifier = lit.add("fun")}},
        {.str = "1234", .should_fail = true},
        {.str = "", .should_fail = true},
        {.str = "+-", .should_fail = true},
    ...
    for (auto c : cases) {
        ...
        auto result = l.reset();
        REQUIRE(t.type == c.expected.type);
        REQUIRE(*result.identifiers.get(common::IdentifierID{t.identifier}) == *lit.get(c.expected.identifier));
    }
}
...
TEST_CASE("lexer: multiple tokens", "[lexer]") {
    ...
    std::vector<common::Token> expected{
        ...
        {.type = NOT},
        {.type = EQUALS},
        {.type = IDENTIFIER, .identifier = lit.add("a")},
        {.type = INTEGER, .integer = 1234},
        {.type = FLOAT, .floating = 1234.1234},
        {.type = BITWISE_AND},
        {.type = BITWISE_OR},
        {.type = IDENTIFIER, .identifier = lit.add("a")},
        {.type = NOT},
        {.type = BOOL, .boolean = true},
        {.type = IDENTIFIER, .identifier = lit.add("bbb")},
    };
    ...
    for (size_t i = 0; i < result.tokens.size(); ++i) {
        ...
        case BOOL: REQUIRE(result.tokens[i].identifier == expected[i].identifier); break;
        case INTEGER: REQUIRE(result.tokens[i].integer == expected[i].integer); break;
        case FLOAT: REQUIRE(std::abs(result.tokens[i].floating - expected[i].floating) < epsilon); break;
        case IDENTIFIER: REQUIRE(*result.identifiers.get(result.tokens[i].identifier) == *lit.get(expected[i].identifier)); break;
        }
    }
}
