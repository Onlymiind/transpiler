...
namespace codegen {
    ...
    Generator::Generator(common::Global &global, common::Module &mod,
        ...
        program_.identifiers = std::move(identifiers);
    ...
    bool Generator::codegen() {
        ...
        for (auto &type : global_types) {
            type_to_idx_[type.get()] = program_.types.size();
            program_.types.emplace_back(std::move(type));
        }
        ...
        program_.global_init = std::move(output_);
    ...
    bool Generator::generate_pointer_maps() {
        program_.type_infos.reserve(program_.types.size());
    ...
    Generator::codegen_index_expression(const common::IndexExpression &expr,
        ...
        push_op(vm::Op::INDEX_ARRAY, type_to_idx_.at(type));
        if (!want_ptr) {
            push_read(expr.type());
        }
        return true;
