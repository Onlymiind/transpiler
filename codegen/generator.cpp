...
namespace codegen {
    bool Generator::codegen() {
        auto &global_types = global_->types();
        for (auto &type : global_types) {
            type_to_idx[type.get()] = program.types.size();
            program.types.emplace_back(std::move(type));
        }

        auto &structs = mod_->get_structs();
        ...
        for (const auto [name, funcID] : mod_->functions()) {
            ...
            func_to_idx[funcID] = func.is_native
                                      ? program.native_functions.size()
                                      : program.functions.size();

            func_ids.push_back(funcID);
            ...
            if (!func.is_native) {
                program.functions.emplace_back(
                    vm::Function{.name = *identifiers_->get(name),
                                 .args = std::move(args)});
            } else {
                program.native_functions.emplace_back(
                    vm::NativeFunction{.name = *identifiers_->get(name),
                                       .args = std::move(args),
        ...
        // in the end, copy the generated bytecode to the appropriate place
        program.global_init = std::move(output);

        // now generate all the functions
        for (const auto funcID : func_ids) {
            const common::Function &func = *ast_->get_function(funcID);
    ...
    bool Generator::codegen_unary(const common::UnaryExpression &expr,
        ...
        switch (expr.op()) {
        case NOT: push_op(vm::Op::NOT, expr.type()->size()); break;
        case NEGATE:
    ...
    bool Generator::codegen_binary(const common::BinaryExpression &expr) {
        ...
        case NOT_EQUALS:
            ...
            break;
        case LESS:
            push_arithmetic_op(expr.type(), vm::Op::LESS_I, vm::Op::LESS_F);
            break;
        case GREATER:
            push_arithmetic_op(expr.type(), vm::Op::GREATER_I,
                               vm::Op::GREATER_F);
            break;
        case LESS_EQUALS:
            push_arithmetic_op(expr.type(), vm::Op::GREATER_I,
    ...
    bool Generator::codegen_function(const common::Function &func) {
        ...
        program.functions[func_to_idx.at(func.id)].code = std::move(output);
        return true;
    }
    ...
    bool Generator::codegen_call(const common::FunctionCall &call) {
        ...
        if (call.name() == append_name) {
            ...
            for (size_t i = 1; i < args.size(); ++i) {
                ...
                push_op(vm::Op::APPEND, type_to_idx.at(args[i]->type()));
            }
            return true;
        }
        ...
        uint64_t idx = func_to_idx.at(call.id());
        push_op(vm::Op::PUSH, idx);
        if (func.is_native) {
            push_op(vm::Op::NATIVE_CALL);
        } else {
            push_op(vm::Op::CALL);
        }
        return false;
    ...
    bool Generator::push_allocate(const common::Type *type) {
        uint64_t type_idx = type_to_idx.at(type);
    ...
    Generator::codegen_index_expression(const common::IndexExpression &expr,
        ...
        if (type->kind() == common::TypeKind::ARRAY) {
            ...
            const auto *size_field = slice->get_field(program.size_name);
            const auto *data_field = slice->get_field(program.data_name);
        ...
        push_op(vm::Op::INDEX_ARRAY, type_to_idx.at(type));
        if (!want_ptr) {
            push_read(expr.type());
        }
        return true;
    ...
    bool Generator::codegen_var_ref(const common::VariableReference &ref,
        ...
        if (!var_to_idx.contains(var.id)) {
            return false;
        }

        uint64_t idx = var_to_idx.at(var.id);
    ...
    bool Generator::codegen_member_access(const common::MemberAccess &access,
        ...
        push_op(vm::Op::ADD_I, field.offset);
